...
void MAP::Save(char *filename)
{
	...
	fp = fopen (filename, "wb");
		...
		for (long i = 0; i < header.max_objects; i++)
		{
			...
			for (i2 = 0; i2 < object[i].max_triangles; i2++) fwrite (&object[i].triangle[i2], sizeof(MAP_TRIANGLE), 1, fp);
			for (i2 = 0; i2 < object[i].max_textures; i2++) fwrite (&object[i].texture[i2], sizeof(MAP_TEXTURE), 1, fp);
		}

		for (i = 0; i < header.max_entities; i++) fwrite (&entity[i], sizeof(MAP_ENTITY), 1, fp);		
		for (i = 0; i < header.max_lights; i++)
		{
		...
		for (i = 0; i < header.max_sounds; i++) fwrite (&sound[i], sizeof(MAP_SOUND), 1, fp);			
		for (i = 0; i < header.max_items; i++) fwrite (&item[i], sizeof(MAP_ITEM), 1, fp);
	...
	if (fp == NULL) return (false);
		...
		if (header.max_objects > 0)
		{
			...
			for (long i = 0; i < header.max_objects; i++)
			{
				...
				for (i2 = 0; i2 < object[i].max_triangles; i2++) fread (&object[i].triangle[i2], sizeof(MAP_TRIANGLE), 1, fp);
				for (i2 = 0; i2 < object[i].max_textures; i2++) fread (&object[i].texture[i2], sizeof(MAP_TEXTURE), 1, fp);
			}
		}
		else object = NULL;

...
/* Constants *****************************************************************************************************/
#define MAX_TEXTURE_NAMES	55
#define MAX_TEXTURE_FILES	50
...
{
	long object;
	long texture[ MAX_TEXTURE_LAYERS ];
} BIND_LIST;
...
BIND_LIST	*bind_list		= NULL;

MULTIPLAYER	multiplayer;
long		connection_type = 0;
char		hostname[50];
long		time_of_last_packet_sent = 0;
...
TEXTURE_FILE texture_file[] =
	{
		...
		{"player_skin.bmp",	false,	GL_LINEAR,	GL_REPEAT},
		
		{"font1_a.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_b.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_c.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_d.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_e.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_f.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_g.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_h.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_i.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_j.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_k.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_l.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_m.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_n.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_o.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_p.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_q.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_r.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_s.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_t.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_u.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_v.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_w.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_x.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_y.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_z.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_0.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_1.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_2.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_3.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_4.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_5.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_6.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_7.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_8.bmp",		true,	GL_NEAREST, GL_REPEAT},
		{"font1_9.bmp",		true,	GL_NEAREST, GL_REPEAT},		
	};
...
void BuildDisplayLists()
{
	...
	{
		glNewList (obj+1, GL_COMPILE_AND_EXECUTE);
			RenderObject (obj);
		glEndList();
	}
}
...
bool LoadMap(char *filename)
{
	...
	if (map.header.max_sounds > 0)
	{
		...
		{			
			audio.SetSource (snd, map.sound[ snd ].filename, (float)map.sound[ snd ].xyz[0], (float)map.sound[ snd ].xyz[1], (float)map.sound[ snd ].xyz[2]);
...
void RenderLight(long cur_light)
{
	...
	for (long obj = 0; obj < map.light[ cur_light ].max_inclusions; obj++)
	{
		...
		glCallList ( map.light[ cur_light ].inclusions[ obj ]+1 );
	}
...
void Render()
{
	...
	if (multiplayer.active)
	{
		glPushMatrix();
			...
			glTranslated (player.xyz[0]+enemy.xyz[0], player.xyz[1]+enemy.xyz[1], player.xyz[2]+enemy.xyz[2]);		
	...
	glTextOut (0, 0, "our game engine");
	glTextOut (0, 15, map.details.map_name);
	CalculateFPS();
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
	switch (msg)
	{
		...
		{			
			...
			multiplayer.Receive();								
			sscanf (multiplayer.incoming_packet, "%i %f %f %f %f %f %f", &packet_type, &x, &y, &z, &xa, &ya, &za);					
			...
			WSAAsyncSelect (multiplayer.game_socket, Window, MM_PACKET_RECEIVED, FD_READ);									
		}break;			
	...
	switch (msg)
	{
		...
		case WM_COMMAND:
		{
			...
			else if (wParam == IDOK)
			{
				...
				if (SendDlgItemMessage (hWnd, IDC_CONFIG_IS_SERVER, BM_GETCHECK, 0, 0) == BST_CHECKED) connection_type = MULTIPLAYER_SERVER;
				else if (SendDlgItemMessage (hWnd, IDC_CONFIG_IS_CLIENT, BM_GETCHECK, 0, 0) == BST_CHECKED)
				{
					GetDlgItemText (hWnd, IDC_CONFIG_HOSTNAME, hostname, 50);
...
void CheckInput()
{
	...
	audio.SetListener ((float)player.xyz[0], (float)player.xyz[1], (float)player.xyz[2], (float)player.angle[0]);		
}
...
void SendMultiplayerPacket()
{
	...
	{		
		sprintf (multiplayer.outgoing_packet,"%i %f %f %f %f %f %f", PACKETTYPE_MOVEMENT, (float)player.xyz[0], (float)player.xyz[1], (float)player.xyz[2], (float)player.angle[0], (float)player.angle[1], (float)player.angle[2]);
		multiplayer.Send ();	
...
int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevious, LPSTR lpCmdString, int CmdShow)
{
	...
	if (connection_type == MULTIPLAYER_SERVER) multiplayer.InitServer();
	else if (connection_type == MULTIPLAYER_CLIENT) multiplayer.InitClient(hostname);
	if (multiplayer.active) WSAAsyncSelect (multiplayer.game_socket, Window, MM_PACKET_RECEIVED, FD_READ);		

	SetGLDefaults();

	LoadMap ("../media/test.map");	
	LoadModel ("../media/item_gun.ase", item_gun);
	LoadModel ("../media/player.ase", player_model);
